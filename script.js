document.addEventListener('DOMContentLoaded', function() {
    ...
    const urlParts = {
        getFullUrl: function() {
            return 'https://docs.google.com/spreadsheets/d/e/2PACX-1vQcK_0NeFjn1-_aV1c9o4rFVDr7VvhYIBnwqTwsW0yY64x2sSErdTIeLOWLqI-2I3ZMvAx-ZoCmWtZE/pub?gid=1674447448&single=true&output=csv';
        }
    };
    ...
    if (!price) return 'N/A';
    let formatted = price.replace(',', '.').replace('.', ',').replace('€', '').trim();
    return `${formatted}€`;
...
<p class="sizes-label">${translations[currentLanguage].availableSizes}</p>
<div class="sizes-list">
    ${shoe.sizes.map(size => `
        <span class="size-badge">
            ${size.size}<span class="size-qty">(${size.quantity})</span>
        </span>
    `).join('')}
</div>
</div>
